...
/**
 * Created by Administrator on 2016/9/7.文件管理器
 */
public class FileManager {
    ...
    public static String inPath = null; //内部路径
    public static String outPath = null; //外部路径
    ...
    //所有文档
    public List<FileBean> filetext;
    //所有文档的大小
    public long filetextsize = 0;
    //所有视频
    public List<FileBean> fileVideo;
    //所有视频的大小
    public long filevideosize = 0;
    //所有音频
    public List<FileBean> fileaudeo;
    //所有音频的大小
    public long fileaudeosize = 0;
    //所有图像
    public List<FileBean> fileImage;
    //所有图像大小
    public long fileImageSize = 0;
    //所有压缩包
    public List<FileBean> filezip;
    //所有压缩包大小
    public long filezipsize = 0;
    //所有安装包
    public List<FileBean> fileapk;
    //所有安装包大小
    public long fileapksize = 0;

    //私有化构造，防止外部new
    private FileManager(){
        fileBeans = new ArrayList<>();
        filetext = new ArrayList<>();
        fileVideo = new ArrayList<>();
        fileaudeo = new ArrayList<>();
        fileImage = new ArrayList<>();
        filezip = new ArrayList<>();
        fileapk = new ArrayList<>();
    }

    public boolean isstop= false;
    public void seleFile(String path,boolean isselect){
        if (path == null){
            return;
        }
        ...
        if (f.isDirectory()){
            ...
            for (int i=0;i<listFiles.length;i++){
                ...
                if (file != null&& file.exists() && file.canRead()&&file.getAbsolutePath() != null){
                    seleFile(file.getAbsolutePath(),false);
                }
            }
        }else { //是文件
            ...
            for (int i=0; i<ICON_TYPE_Table.length;i++){
                if (fileName!=null&&fileName.equals(ICON_TYPE_Table[i][0])){
                    fileBean.setFileIoc(ICON_TYPE_Table[i][1]);
                    fileBean.setFiletype(ICON_TYPE_Table[i][2]);
                }
            }
            if (fileBean.getFileIoc() == null || fileBean.getFiletype() == null){
                return;
            }
            if (fileBean.getFiletype().equals(FileManager.TYPE_TXT)){
                filetext.add(fileBean);
                filetextsize = filetextsize+f.length();
            }else if (fileBean.getFiletype().equals(FileManager.TYPE_VIDEO)){
                fileVideo.add(fileBean);
                filevideosize = filevideosize + f.length();
            }else if (fileBean.getFiletype().equals(FileManager.TYPE_AUDIO)){
                fileaudeo.add(fileBean);
                fileaudeosize = fileaudeosize + f.length();
            }else if (fileBean.getFiletype().equals(FileManager.TYPE_IMAGE)){
                fileImage.add(fileBean);
                fileImageSize = fileImageSize + f.length();
            }else if (fileBean.getFiletype().equals(FileManager.TYPE_ZIP)){
                filezip.add(fileBean);
                filezipsize = filezipsize + f.length();
            }else if (fileBean.getFiletype().equals(FileManager.TYPE_APK)){
                fileapk.add(fileBean);
                fileapksize = fileapksize + f.length();
            }
            ...
            message.what = 1;
            message.obj = fileBean.getFiletype();
    ...
    public String getFileName(String str){
        ...
        return str.substring(idex+1,str.length());
    }
    static {
        inPath = PhoneUtil.getPhoneInsid();  //获得内部路径
        outPath = PhoneUtil.getPhoneOutPath();  //获得外部路径
    }
    //单例
